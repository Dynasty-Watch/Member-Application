...
const Statistic: React.FC = () => {
  ...
  const [drinkingAndDriving, setDrinkingAndDriving] = useState();
  const [drugRelated, setDrugRelated] = useState();
  const [hijacking, setHijacking] = useState();
  ...
  useEffect(() => {
    ...
    getCommercialtCases();
    getPropertyCases();
    getDrinkingAndDrivingCases();
    getDrugRelatedCases();
    getHijackingCases();
    getRobberyCases();
  }, []);

  const getAssaultCases = async () => {
    const { data } = await Supa.rpc("assault_cases").single();
    setAssault(data);
  };

  const getburglaryCases = async () => {
    const { data } = await Supa.rpc("burglary_cases").single();
    setBurglary(data);
  };

  const getCommercialtCases = async () => {
    const { data } = await Supa.rpc("commercial_cases").single();
    setCommercial(data);
  };

  const getPropertyCases = async () => {
    const { data } = await Supa.rpc("destruction_of_property_cases").single();
    setProperty(data);
  };

  const getDrinkingAndDrivingCases = async () => {
    const { data } = await Supa.rpc("drinking_and_driving_cases").single();
    setDrinkingAndDriving(data);
  };

  const getDrugRelatedCases = async () => {
    const { data } = await Supa.rpc("drug_related_cases").single();
    setDrugRelated(data);
  };

  const getHijackingCases = async () => {
    const { data } = await Supa.rpc("hijacking_case").single();
    setHijacking(data);
  };

  const getRobberyCases = async () => {
    const { data } = await Supa.rpc("robbery_cases").single();
    setRobbery(data);
  };
  ...
  const dataSource = {
    ...
    data: [
      ...
      { label: "Drug Related", value: drugRelated },
      { label: "Assault", value: assault },
      { label: "Hijacking", value: hijacking },
      { label: "Commercial Crime", value: commercial },
      { label: "Destruction Of Property", value: property },
      { label: "Drinking And Driving", value: drinkingAndDriving },
    ],
  };
